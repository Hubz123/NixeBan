    ...
    async def lpg_status(self, ctx):
        ...
        try:
            ...
            src = [
                ...
                ('gem_thr', _cfg_origin('GEMINI_LUCKY_THRESHOLD')),
            ]
        except Exception:
            pass
        txt = (f"guard_channels={sorted(getattr(cog,'guard_channels',[]))}\n"
               f"redirect={getattr(cog,'redirect_channel',None)}\n"
               f"delete>={getattr(cog,'min_conf_delete',0):.2f} redirect>={getattr(cog,'min_conf_redirect',0):.2f} gem_thr={getattr(cog,'gemini_lucky_thr',0):.2f}\n"
