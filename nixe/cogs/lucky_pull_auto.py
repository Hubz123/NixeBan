...
# shared helpers (duplicated small snippet to avoid import loops)
...
class LuckyPullAuto(commands.Cog):
    ...
    async def _on_message(self, msg: discord.Message):
        ...
        for a in images:
            try:
                datas.append(await a.read(use_cached=True))
            except Exception:
                pass
        if not datas:
            return

        # build lightweight hints from message text (helps classifier focus)
