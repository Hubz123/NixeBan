...
class NixeBanCommands(commands.Cog):
    ...
    def _logch(self, guild: discord.Guild) -> Optional[discord.TextChannel]:
    ...
    async def testban_cmd(self, ctx: commands.Context, *, args: str = ""):
        ...
        if not ch:
            return await ctx.reply("Set ENV `NIXE_BAN_LOG_CHANNEL_ID`.", mention_author=False)
        evidence_url = None
    ...
    async def ban_cmd(self, ctx: commands.Context, *, args: str = ""):
        ...
        if dry:
            reason = re.sub(r"\s*--dry\b", "", reason).strip() or "—"
