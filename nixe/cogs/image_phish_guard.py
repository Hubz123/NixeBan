\
import os, io, logging, re, time, datetime as dt
...
appcfg = load()
cfg = appcfg.image
AUTO_CREATE = (os.getenv('AUTO_CREATE_DB_THREAD','1').lower() in ('1','true','yes','on','y'))
...
MOD_LOG_CHANNEL_NAME = os.getenv('MOD_LOG_CHANNEL_NAME', appcfg.mod_log_channel_name)

MAGIC_WEBP = b"RIFF"; MAGIC_PNG = b"\x89PNG\r\n\x1a\n"; MAGIC_JPEG = b"\xff\xd8\xff"
...
def _img_to_ahash(img: "Image.Image", size: int=8) -> int:
    g = img.convert("L").resize((size, size)); px = list(g.getdata()); avg = sum(px)/len(px); bits = 0
    for i,p in enumerate(px):
        if p > avg: bits |= (1 << i)
    return bits

def _img_to_dhash(img: "Image.Image", size: int=8) -> int:
    g = img.convert("L").resize((size+1, size)); bits = 0; i = 0
    for y in range(size):
        for x in range(size):
            left = g.getpixel((x,y)); right = g.getpixel((x+1,y))
...
def _hash_image_bytes(b: bytes) -> Optional[int]:
    if Image is None:
        log.error("Pillow not available; cannot compute image hash"); return None
    try:
        with Image.open(io.BytesIO(b)) as im:
            im = im.convert("RGB")
            try: return _img_to_dhash(im, 8)
            except Exception: return _img_to_ahash(im, 8)
    except Exception as e:
        log.warning("Failed to open image for hashing: %s", e); return None
...
class PhashDB:
    ...
    def add_hex(self, hx: str):
        ...
        try: self.hashes.add(int(hx,16))
        except Exception: pass
    def match(self, h: int, strict: int, lenient: int):
        # return ("ban"/"quarantine"/None, dist, ref)
        best = None
        for ref in self.hashes:
            d = _hamming(h, ref)
            if best is None or d < best[0]: best = (d, ref)
    ...
    def parse_from_text(txt: str) -> "PhashDB":
        db = PhashDB()
        for m in re.finditer(r"\b([0-9a-fA-F]{16})\b", txt): db.add_hex(m.group(1))
        for m in re.finditer(r"phash\s*[:=]\s*([0-9a-fA-F]{16})", txt, re.I): db.add_hex(m.group(1))
        return db
...
class ImagePhishGuard(commands.Cog):
    ...
    async def refresh_db_task(self):
        try:
            ...
            if loaded: log.info("[db] loaded %d entries from %s", len(self.db), self.db.loaded_from)
        except Exception as e:
    ...
    async def _load_db(self) -> bool:
        ...
        if cfg.db_channel_id:
            channel = self.bot.get_channel(cfg.db_channel_id)
        # fallback by name
        if channel is None and PHISH_DB_CHANNEL_NAME:
            for g in self.bot.guilds:
                channel = await find_text_channel(g, channel_name=PHISH_DB_CHANNEL_NAME)
                if channel: break
        if channel is None:
            return False
        # Resolve thread by ID/name
        if cfg.db_thread_id:
            thread = self.bot.get_thread(cfg.db_thread_id)
        ...
        # Create thread if missing and allowed
        created = False
        ...
        if thread is None:
            return False
        # Load from thread pins/history
        target = thread
        try:
            pins = []
            try: pins = await target.pins()
            except Exception: pass
            texts = []
            if pins:
                for m in pins:
                    ...
                    for a in m.attachments:
                        if a.size and a.size < 2_000_000 and (a.content_type or "").startswith("text/"):
                            b = await a.read(); texts.append(b.decode("utf-8","ignore"))
            else:
            ...
            if len(db) > 0:
                self.db = db; self.db.loaded_from = f"thread:{target.id}"; return True

# If freshly created and still empty, pin a template for admin to fill
if created:
    try:
        ...
        log.warning("Failed to pin template to new thread: %s", e)
            ...
            log.exception("load_db error: %s", e); return False
    ...
    def _ban_ceiling_hit(self) -> bool:
        now = time.time()
        self._ban_window = [t for t in self._ban_window if now - t < 600]  # last 10 min
    ...
    async def _modlog(self, guild: discord.Guild, text: str):
        try:
            app = load(); ch_id = app.mod_log_channel_id; ch = None
            if ch_id:
                ch = guild.get_channel(ch_id) or (await guild.fetch_channel(ch_id))
            if ch is None and app.mod_log_channel_name:
                for c in guild.text_channels:
                    if c.name.lower() == app.mod_log_channel_name.lower():
                        ch = c; break
            if ch:
                await ch.send(text)
        except Exception:
    ...
    async def _act(self, message: discord.Message, verdict: str, dist: int, hhex: str):
        ...
        if verdict == "ban":
            if not message.guild.me.guild_permissions.ban_members:
                await self._modlog(message.guild, f"[ban-skip] missing Ban Members perms for {message.author}"); return
            try:
                await message.author.ban(reason=f"image-phish match (dist={dist}, hash={hhex})",
                                         delete_message_days=cfg.delete_message_days)
                self._last_ban_ts = time.time(); self._ban_window.append(self._last_ban_ts)
    ...
    async def phishguard_cmd(self, ctx: commands.Context, sub: str = "status"):
        if sub == "reload":
            ...
            return
        await ctx.reply(f"status: entries={len(self.db)} from={self.db.loaded_from or 'n/a'}  "
