...
from ..config.self_learning_cfg import LOG_CHANNEL_ID, PHASH_INBOX_THREAD, PHASH_HAMMING_MAX, PHASH_DB_MARKER
...
class ImagePhishGuard(commands.Cog):
    ...
    async def _load_db_hashes(self, guild: discord.Guild) -> List[str]:
        ...
        async for m in ch.history(limit=100):
            if PHASH_DB_MARKER in (m.content or ''):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not self._is_inbox(thread): return
        ...
        if not imgs: return
        try: raw = await imgs[0].read()
        except Exception: return
        h = _compute_phash(raw)
        if not (h and HEX16.match(h)): return
        db_hashes = await self._load_db_hashes(message.guild)
        if not db_hashes: return
        matched = (h in db_hashes) if (PHASH_HAMMING_MAX <= 0) else any(len(dh)==len(h) and _hamm(h,dh) <= PHASH_HAMMING_MAX for dh in db_hashes)
