...
from ..config.self_learning_cfg import LOG_CHANNEL_ID, PHASH_INBOX_THREAD, PHASH_DB_MARKER, PHASH_WATCH_FIRST_DELAY, PHASH_WATCH_INTERVAL
log = logging.getLogger(__name__)
try:
    from PIL import Image as _PIL_Image
except Exception: _PIL_Image = None
try:
    import imagehash as _imagehash
except Exception: _imagehash = None
HEX16 = re.compile(r'^[0-9a-f]{16}$', re.I)
BLOCK_RE = re.compile(r'(?:^|\n)\s*%s\s*```(?:json)?\s*(\{.*?\})\s*```' % re.escape(PHASH_DB_MARKER), re.I | re.S)
def _compute_phash(raw: bytes) -> Optional[str]:
    if _PIL_Image is None or _imagehash is None: return None
    try:
...
def _norm_list(obj) -> List[str]:
    out: List[str] = []
    def push(x):
        if isinstance(x, str) and HEX16.match(x.strip()): out.append(x.strip())
    if isinstance(obj, dict):
        arr = obj.get('phash') or obj.get('items') or []
        if isinstance(arr, list):
            for it in arr: push(it if isinstance(it, str) else (it.get('hash') if isinstance(it, dict) else None))
...
class NixePhashInboxWatcher(commands.Cog):
    ...
    async def _get_inbox_thread(self, ch: discord.TextChannel) -> Optional[discord.Thread]:
        ...
        for t in list(ch.threads):
            if isinstance(t, discord.Thread) and t.name.lower() in wanted: return t
        try:
    ...
    async def _get_or_create_db_message(self, container: discord.abc.Messageable) -> Optional[discord.Message]:
        try:
            async for msg in container.history(limit=100):
                if isinstance(msg.content, str) and PHASH_DB_MARKER in msg.content and BLOCK_RE.search(msg.content): return msg
        except Exception: pass
        try:
    ...
    def _summary_embed(self, total: int) -> discord.Embed:
        e = discord.Embed(title='pHash DB (imagephising)', description=f'Total unik: **{total}**\n\n_Disimpan pada pesan ini; selalu di-EDIT (no spam)._', color=0x2e8b57)
        e.set_footer(text='NIXE • pHash inbox watcher'); return e
